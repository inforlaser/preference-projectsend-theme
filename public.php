...
<body class="bg-cream-100 min-h-screen flex flex-col text-slate-700">
    ...
    <main class="mx-auto flex-1 w-full max-w-360 px-4 py-3 sm:px-6 sm:py-4">
            ...
            <div class="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                <div>
                    ...
                    <p class="text-sm text-slate-500">
                        <?php echo (int) $count; ?> <?php _e('files published for direct access.', 'preference_template'); ?>
                    </p>
                </div>
            </div>
        </div>
        ...
        <?php if ($count > 0 || isset($_GET['search']) || isset($_GET['category'])): ?>
            <div class="mb-6 rounded-3xl border border-cream-200 bg-white p-5 shadow-sm shadow-cream-200/40 sm:p-6">
                <form action="<?php echo BASE_URI; ?>public.php" method="get"
                            ...
                            <div class="shrink-0 lg:ml-auto">
                                <?php if ($file->expired == false) { ?>
                                    <div class="flex flex-wrap gap-2">
                                        <a href="<?php echo BASE_URI; ?>download.php?id=<?php echo $file->id; ?>&token=<?php echo $file->public_token; ?>" target="_blank"
                                ...
                                <?php } else { ?>
                                    <div class="file_actions_notice rounded-md bg-red-50 px-4 py-2 text-sm font-medium text-red-600">
